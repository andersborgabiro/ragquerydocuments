...
def main():
    # Application header
    st.set_page_config(
        page_title="Query Documents",
        page_icon="🙋‍♂️",
        layout="centered",
        menu_items={"About": "https://abiro.com"},
    )
    st.title("📄 Query Documents")
    st.markdown(
        "Upload your documents (PDF, Word, PowerPoint, Excel, Text, Markdown, HTML, XML or JSON), and ask questions based on their content. The system will only use information from the uploaded files to answer your queries. The provided API Key, set via OPENAI_API_KEY or manually, is only used for queries and not otherwise stored."
    )
    ...
    if openai_api_key == "":
        ...
        openai_api_key = st.text_input(
            "Enter your API key (not shown):", type="password"
        )
        st.button("Set")
    ...
    selected_embedding_display = st.sidebar.selectbox(
        "Embedding Model",
        list(embedding_options.keys()),
        index=selected_embedding_display_default,
    )
    selected_embedding_value = embedding_options[selected_embedding_display]

    # RAG chunk size
    rag_chunk_size_default = 1000
    rag_chunk_size = st.sidebar.number_input(
        "Embedding Chunk Size",
        min_value=100,
        max_value=10000,
        step=100,
        value=rag_chunk_size_default,
    )

    # RAG chunk overlap
    rag_chunk_overlap_default = 100
    rag_chunk_overlap = st.sidebar.number_input(
        "Embedding Chunk Overlap",
        min_value=10,
        max_value=1000,
        step=10,
        value=rag_chunk_overlap_default,
    )
    ...
    selected_gpt_display = st.sidebar.selectbox(
        "GPT Model", list(gpt_options.keys()), index=selected_gpt_display_default
    )
    ...
    temperature = st.sidebar.slider(
        "GPT Temperature", 0.0, 1.0, temperature_default, 0.1
    )
    ...
    st.session_state.verbose = st.sidebar.checkbox(
        "Verbose",
        help="Check this if you want intermediate information and a timetrace",
    )

    # Initialize session state for embeddings and FAISS index
    if "embeddings" not in st.session_state:
        st.session_state.embeddings = OpenAIEmbeddings(
            model=selected_embedding_value, openai_api_key=openai_api_key
        )
    ...
    uploaded_files = st.sidebar.file_uploader(
        "Choose files",
        type=["pdf", "txt", "md", "docx", "pptx", "xlsx", "html", "htm", "json", "xml"],
        ...
        for uploaded_file in uploaded_files:
            if uploaded_file.name not in st.session_state.processed_files:
                ...
                if text:
                    ...
                    st.sidebar.error(
                        f"Failed to extract text from {uploaded_file.name}."
                    )
            else:
